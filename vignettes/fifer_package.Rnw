...
\subsection{Code}
<<echo=FALSE>>=
options(prompt=" ", width=50, continue=" ")
@
...
\texttt{string.to.color} & Given a vector of group labels (e.g., ``male", ``female", ``female", ``male", etc.)   \\
		& \texttt{string.to.color} will automatically generate a vector of colors to correspond to \\
		& the group labels. \\
 \hline
\end{tabular}
\end{center}
